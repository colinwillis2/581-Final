...
TarHeader extends Object
{
            ...
            throws InvalidHeaderException
    {
        ...
        mode = (int)
                parseOctal(header, offset, MODELEN);

        offset += MODELEN;

        userId = (int)
                parseOctal(header, offset, UIDLEN);

        offset += UIDLEN;

        groupId = (int)
                parseOctal(header, offset, GIDLEN);
        ...
        modTime =
                parseOctal(header, offset, MODTIMELEN);

        offset += MODTIMELEN;

        checkSum = (int)
                parseOctal(header, offset, CHKSUMLEN);
        ...
        magic =
                parseName(header, offset, MAGICLEN);

        offset += MAGICLEN;

        userName =
                parseName(header, offset, UNAMELEN);

        offset += UNAMELEN;

        groupName =
                parseName(header, offset, GNAMELEN);

        offset += GNAMELEN;

        devMajor = (int)
                parseOctal(header, offset, DEVLEN);
